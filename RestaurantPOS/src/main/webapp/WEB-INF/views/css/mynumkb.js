...
(function($){
	...
	Mynumkb.prototype = {
            ...
            var _html = [
                '<div class="mykb-box" id="mykeyboard'+me.count+'">',
                    '<ul class="num-key clearfix">',
                        ...
                        '<li class="num">5</li>',
                        '<li class="num">6</li>',
                        '<li class="func del">清除</li>',
                        '<li class="num">7</li>',
                        '<li class="num">8</li>',
                        '<li class="num">9</li>',
                        '<li class="func clearall">清除全部</li>',
                        '<li class="num">0</li>',
                        '<li class="num">.</li>',
                        '<li class="func sure">確定</li>',
                    '</ul>',
                '</div>',
            ].join("");
        ...
        _liclick:function(e){
            var me = this;
            var $target = $(e.target);
            if($target.hasClass("del")){//���聢
                me.setValue("del");
            }else if($target.hasClass("exit")||$target.hasClass("sure")){//���枂--蝖桀��
                me.close();
            }else if($target.hasClass("clearall")){//皜��膄
                me.$element.val("");
            }else{//颲枏�亙�嗡�𡝗㺭摮�
                var str = $target.text();
                me.setValue("add",str);
        ...
        getCursorPosition:function(){
            ...
            if($element.selectionStart!=undefined && $element.selectionStart!=null){//��燶E瘚讛��膥
