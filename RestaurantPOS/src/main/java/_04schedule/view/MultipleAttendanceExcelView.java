...
public class MultipleAttendanceExcelView extends AbstractXlsView  {
	...
	private void populateExcelCells(Map<String, Object> model, Workbook workbook) {
        ...
        List<AttendenceBean> attendenceBeans = (List<AttendenceBean>) model.get("allAttendenceBeans");
        
		Set<String> set = model.keySet();
		Row row = null;
		Cell cell = null;
		for(AttendenceBean m : attendenceBeans) {
			colCount = 0;
...
//			cell.setCellValue(m.getpId());
			...
			cell = row.createCell(colCount++);
			cell.setCellStyle(styleName);
			cell.setCellValue(m.getCheckStatus());
			
			cell = row.createCell(colCount++);
			cell.setCellStyle(styleName);
			cell.setCellValue(m.getClockTime());
