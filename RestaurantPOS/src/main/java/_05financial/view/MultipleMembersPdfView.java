...
//		processArticle(document);
	...
	private void processTable(Map<String, Object> model, Document document, int newLines) throws Exception {
		...
		addTitle(document, "類別報表", titleFont);
		document.add(Chunk.NEWLINE);
		String[] tableTitle = {"數量", "銷售金額"};
		PdfPTable table = createTable(model, tableTitle, 2);
		document.add(table);
...
//			table.addCell(cell4);
	...
	public String fileToString(String textFile, String charEncoding) throws Exception {
		String msg = "";
		try (InputStream fis = context.getResourceAsStream(textFile);
